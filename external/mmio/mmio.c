...
int mm_read_banner(FILE *f, MM_typecode *matcode)
{
    ...
    if (sscanf(line, "%s %s %s %s %s", banner, mtx, crd, data_type, storage_scheme) !=
        5)
        return MM_PREMATURE_EOF;
    ...
    FILE *f, int M, int N, int nz, int I[], int J[], double val[], MM_typecode matcode)
{
    ...
    if (mm_is_complex(matcode))
    {
        for (i = 0; i < nz; i++)
            if (fscanf(
                    f, "%d %d %lg %lg", &I[i], &J[i], &val[2 * i], &val[2 * i + 1]) !=
                4)
                return MM_PREMATURE_EOF;
    }
...
int mm_read_mtx_crd(
    char *fname, int *M, int *N, int *nz, int **I, int **J, double **val,
    MM_typecode *matcode)
{
    int ret_code;
    FILE *f;
